...
More details about the specific files and the Airflow runtine can be found in their respective folders in the repository. 
...
As can be surmised by the way our architecture diagram is layed out, we decided to deliver two types of predictions (recommendations), online and batch. The output of the batch predictions can be found in the respective folders of each model, and are output in CSV files. We have also uploaded a version of these into the DAG folder of the repository so that the results of a sample run can be seen. These contain the predictions for the latest run of the Airflow DAG for the latest state of the database, as the entire Airflow system does the entire process from gathering data from the API to generating the final batch predictions. 

As for the online predictions, these are done somewhat differently. We deploy the models to VertexAI endpoints, and we then use those models to serve the predictions. We tested this implementation in some python notebooks that run inside the JupyterLab instance in the VertexAI workbench. 

In terms of the manual components of the product, the only ones that are manual are those related to online predictions. There is a python notebook that we run that contains all the startup scripts and deployment details to create the models in VertexAI, and there is the notebook that we use to test these online predictions. The only truly manual part of this process is the undeployment of the models, as those have to be manually removed after online predictions are served to save on costs. 

As for batch predictions, everything is fully automatic. We can trigger the DAG to run at any point manually too, but it is not necessary, as the instance turns on and runs once every 12 hours. 

## Reflections & Post Mortem

What we learned in the course of this project could fill a book, so we will only highlight some of the particular things that worked and those that did not. 

In terms of what did work, the application and each of the architectural steps and decisions we took were good. If we were to do this project again, we might try to do everything in VertexAI or everything through the DAg and concentrate on either bath or online, but doing both really helped us understand the difference and the choices that we have to make when building a production environment. We also worked very well as a team, and we felt supported and like our core skills complenented each other in interesting ways. 

The thing that is most glaring in terms of what did not work was the information that we chose. We could have gone for a much, much bigger database or one of recipies in general, like the previously mentioned [Edamam recipe database]([Over 2 Mllion Recipes - Recipe Search API - Edamam](https://developer.edamam.com/edamam-recipe-api)), but we were worried we would not be able to handle it, and decided for a smaller base. We should have gone for it, as the final product could have been a lot more interesting than what we managed to generate with very few data points. 

From this, the next step is clear, and it is to try and find more varied data and do some feature engineering so that we either adapt our models to that data or we adapat the data to the structure that the models accept. Other than that, we as with any work in progress, we left some things on the table. Of particular note would be the decoupling of the PCA and the clustring parts of the data product, and the script that we would need to undeploy models from VertexAI. these are small things in the larger scheme of the project, but would have been nice things to have. 

Something else that deserves special mention was the tech stack and how well it worked. Working on GCP has been a charm, and things the UX design and the particular decisions taken by that cloud team really did help us deploy models and create a very complete data product in a relatively short span of time, especially considering most of us had not really worked with these tools before in any capacity. 

## Broader Impacts

One unintended impact is something we detailed in our pre-mortem, and that is the dangers of working with cloud architectures if you are not careful with credentials. We had a data breach through uploading an auth file to GitHub, and we had to restart the project from scratch. It was not easy, and it was very time consuming to do so. One thing that we would like to highlight is the importance of this and cybersecurity good practices in general. 

As for the application itself, we have developed a very good skeleton for any type of recommendation system based on ingredients. This can be adapted in any number of ways, the ammout of clusters shifted depending on the speciif problem, but what we have is the framework for something that can one day be deployed and develop into a real application. One pottential example is [this article about clustering systems for diabetic patients](https://ieeexplore.ieee.org/document/5480416), something that can with a little work be adapted onto our application without much hassle. To us, this turned out to be more than just a project, but an entire, working, serious end to end machine learning solution, and that is something we were very proud to be able to accomplish. 
